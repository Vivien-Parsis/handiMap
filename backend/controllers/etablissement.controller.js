...
const getEtablissementWithAllJoin = async (req, res) => {
    ...
    try {
        ...
        const etablissement = await etablissementModel.findOneWithAllJoin(req.query.id_etablissement)
        const transformRes = []
...
const getAllEtablissementWithAllJoin = async (req, res) => {
    try {
        const etablissement = await etablissementModel.findAllWithAllJoin()
        const transformRes = []
        for (let etab of etablissement) {
            if (!transformRes.some(e => e.id_etablissement == etab.id_etablissement)) {
                transformRes.push({
...
export {
    ...
    getAllEtablissementWithAllJoin
}